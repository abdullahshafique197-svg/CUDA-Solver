...
!-------------------------AI file write variables ----------------------
double precision,dimension(nP(1),nP(2),nP(3))		:: wdudz, wdvdz ! for writing of dudz and dvdz at walls
integer																					:: cntstrt,cntr,nstepw=6160,fnum,jcnt,icnt,jjcnt
character(len=30)      													:: filenameAI,filenameAIp
...
!***********************WRITING FILES FOR 'AI' (starts)*****************
  if(mod(nstep,nstepw)==0) then
    !-------------------------------------------------------------------
		cntstrt = 2069
		cntr = nstep/nstepw
		fnum = cntr-cntstrt
    ...
    if(bottom) then
      ...
      do j=bs(2),es(2)
        do i=bs(1),es(1)
          wdudz(i,j,1) = 					  (     f1d2c0*var(i,j,1,2)  &
                         			 	        + f1d2c1*var(i,j,2,2)  &
                               		  		+ f1d2c2*var(i,j,3,2)  &
                     					      )

          wdvdz(i,j,1) = 					  (     f1d2c0*var(i,j,1,3)  &
                     					          + f1d2c1*var(i,j,2,3)  &
                     					          + f1d2c2*var(i,j,3,3)  &
                     					      )
        enddo
      ...
      WRITE(filenameAIp,'(a,i3.3,a)')"../output/data/bot",fnum,"pw.dat"        
      OPEN (UNIT=10,FILE=filenameAIp)

       write(10,103) 'TITLE ="',time,nstep,'"'
       write(10,*) 'Variables ="x","y","Pw","dudz","dvdz"'
       write(10,1041) 'ZONE j=',es(2)-bs(2)+1,',i=',es(1)-bs(1)+1,',DATAPACKING="POINT"'

        do j=1,np(2)
         write(10,*)
            do i=1,np(1)
	          	 write(10,200) x1(i),x2(j),var(i,j,1,7),wdudz(i,j,1),wdvdz(i,j,1)
            enddo
        enddo

      close(10)  

      WRITE(filenameAI,'(a,i3.3,a)')"../output/data/",fnum,".dat"        
      OPEN (UNIT=10,FILE=filenameAI)
       ...
       do j=5,125,5
         ...
         do i=5,125,5
        	 do jcnt = -6,6,1
        	  do icnt = -2,2,1
        	  	jjcnt =j+jcnt
        		  if (j.eq.5.and.jcnt.eq.-6)  jjcnt = np(2)-1
        	  	if (j.eq.5.and.jcnt.eq.-5)  jjcnt = np(2)
        		  if (j.eq.125.and.jcnt.eq.6) jjcnt = 1
	          	 write(10,200) x1(i+icnt),x2(jjcnt),var(i+icnt,jjcnt,1,7),wdudz(i+icnt,jjcnt,1),wdvdz(i+icnt,jjcnt,1),var(i,j,16,4)
            enddo
           enddo
         end do
       end do
      ...
      close(10)  
!      103  format(A,F20.10,I9,A)
      1041  format(A,I3,A,I3,A)
      200  format(2(1X,F10.6),4(1X,E22.15))
    endif
    ...
    if(top) then
      ...
      do j=bs(2),es(2)
        do i=bs(1),es(1)
          ...
          wdudz(i,j,np(3)) =  		(       b1d2c0*var(i,j,nP(3)  ,2)  &
                              			    + b1d2c1*var(i,j,nP(3)-1,2)  &
                              			    + b1d2c2*var(i,j,nP(3)-2,2)  &
                               		)

          wdudz(i,j,np(3)) = 		  (       b1d2c0*var(i,j,nP(3)  ,3)  &
    ...
    if(top) then
      WRITE(filenameAI,'(a,i3.3,a)')"../output/data/",fnum,".dat"
      open(10,file=filenameAI,access="append")
          	  ...
          	  do icnt = -2,2,1
          	   jjcnt = j+jcnt
        	  	 ...
        	  	 if (j.eq.125.and.jcnt.eq.6) jjcnt = 1
	            	...
	            	write(10,200) x1(i+icnt),x2(jjcnt),var(i+icnt,jjcnt,np(3),7),wdudz(i+icnt,jjcnt,np(3)), & 		
	          											  																				 wdvdz(i+icnt,jjcnt,np(3)),var(i,j,np(3)-15,4)
              enddo
            enddo
          enddo
        enddo
