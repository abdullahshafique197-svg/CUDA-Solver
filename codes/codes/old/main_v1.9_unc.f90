...
double precision 			:: df21,df31,df32,df41,df42,df43
double precision 			:: db12,db13,db23,db14,db24,db34
!--------------------------Temporary Variables--------------------------
				...
				if(bottom) then
					if (i.eq.1)  then
							...
							dudzdx	= (	(		f1d2c0*mu(i,j,1)*var(i+1,j,1,2)	&
													...
													(		f1d2c0*mu(i,j,1)*var(i	,j,1,2)	&
														+ f1d2c1*mu(i,j,2)*var(i	,j,2,2)	&
														+ f1d2c2*mu(i,j,3)*var(i	,j,3,2)	&
													)																		&
												) *invdx

					elseif (i.eq.nP(1)) then
							dudxdz	= (	(		mu(i	,j,1)*(		f1d2c0*var(i	,j,1,2)	&
																						+ f1d2c1*var(i	,j,2,2)	&
																						+ f1d2c2*var(i	,j,3,2)	&
																					)													&
													) -																				&
							...
							dudzdx	= (	(		f1d2c0*mu(i,j,1)*var(i	,j,1,2)	&
														+ f1d2c1*mu(i,j,2)*var(i	,j,2,2)	&
														+ f1d2c2*mu(i,j,3)*var(i	,j,3,2)	&
													) -																	&
					...
					if (j.eq.1) then
						...
						dvdzdy	= (	(		f1d2c0*mu(i,j,1)*var(i,j+1,1,3)	&
												...
												(		f1d2c0*mu(i,j,1)*var(i,j	,1,3)	&
													+ f1d2c1*mu(i,j,2)*var(i,j	,2,3)	&
													+ f1d2c2*mu(i,j,3)*var(i,j	,3,3)	&
												)																		&
											) *invdy

					elseif (j.eq.nP(2)) then
						dvdydz	= (	(		mu(i,j	,1)*(		f1d2c0*var(i,j	,1,3)	&
																					+ f1d2c1*var(i,j	,2,3)	&
																					+ f1d2c2*var(i,j	,3,3)	&
																				)													&
												) -																				&
						...
						dvdzdy	= (	(		f1d2c0*mu(i,j,1)*var(i,j	,1,3)	&
													+ f1d2c1*mu(i,j,2)*var(i,j	,2,3)	&
													+ f1d2c2*mu(i,j,3)*var(i,j	,3,3)	&
												) -																	&
					...
					lhs			= invRe*(-dudxdz -dvdydz + two3rd*(dudzdx + dvdzdy - two*(mud2wdz2 + dwdzdmudz))) + drhow2dz
				...
				if(top) then
					if (i.eq.1) then
						dudxdz	= (	(		mu(i+1,j,nP(3)  )*(		b1d2c0*var(i+1,j,nP(3)  ,2)	&
												...
												(		mu(i	,j,nP(3)  )*(		b1d2c0*var(i	,j,nP(3)  ,2)	&
						...
						dudzdx	= (	(		b1d2c0*mu(i,j,nP(3)  )*var(i+1,j,nP(3)  ,2)	&
												...
												(		b1d2c0*mu(i,j,nP(3)  )*var(i	,j,nP(3)  ,2)	&
													+ b1d2c1*mu(i,j,nP(3)-1)*var(i	,j,nP(3)-1,2)	&
													+ b1d2c2*mu(i,j,nP(3)-2)*var(i	,j,nP(3)-2,2)	&
												)																								&
											) *invdx
					elseif (i.eq.nP(1)) then
						dudxdz	= (	(		mu(i	,j,nP(3)  )*(		b1d2c0*var(i	,j,nP(3)  ,2)	&
																								+ b1d2c1*var(i	,j,nP(3)-1,2)	&
																								+ b1d2c2*var(i	,j,nP(3)-2,2)	&
																							)																&
												)	-																										&
						...
						dudzdx	= (	(		b1d2c0*mu(i,j,nP(3)  )*var(i	,j,nP(3)  ,2)	&
													+ b1d2c1*mu(i,j,nP(3)-1)*var(i	,j,nP(3)-1,2)	&
													+ b1d2c2*mu(i,j,nP(3)-2)*var(i	,j,nP(3)-2,2)	&
												)	-																							&
					...
					if (j.eq.1) then
						dvdydz	= (	(		mu(i,j+1,nP(3)  )*(		b1d2c0*var(i,j+1,nP(3)  ,3)	&
												...
												(		mu(i,j	,nP(3)  )*(		b1d2c0*var(i,j	,nP(3)  ,3)	&
						...
						dvdzdy	= (	(		b1d2c0*mu(i,j,nP(3)  )*var(i,j+1,nP(3)  ,3)	&
												...
												(		b1d2c0*mu(i,j,nP(3)  )*var(i,j	,nP(3)  ,3)	&
													+ b1d2c1*mu(i,j,nP(3)-1)*var(i,j	,nP(3)-1,3)	&
													+ b1d2c2*mu(i,j,nP(3)-2)*var(i,j	,nP(3)-2,3)	&
												)																								&
											)	*invdy
						elseif (j.eq.nP(2)) then
							dvdydz	= (	(		mu(i,j	,nP(3)  )*(		b1d2c0*var(i,j	,nP(3)  ,3)	&
																									+	b1d2c1*var(i,j	,nP(3)-1,3)	&
																									+ b1d2c2*var(i,j	,nP(3)-2,3)	&
																								)																&
													)	-																										&
							...
							dvdzdy	= (	(		b1d2c0*mu(i,j,nP(3)  )*var(i,j	,nP(3)  ,3)	&
														+	b1d2c1*mu(i,j,nP(3)-1)*var(i,j	,nP(3)-1,3)	&
														+ b1d2c2*mu(i,j,nP(3)-2)*var(i,j	,nP(3)-2,3)	&
													)	-																							&
